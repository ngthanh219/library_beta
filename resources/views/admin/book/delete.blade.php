...
@section('index')
    <div class="content-wrapper" id="formContent">
        <section class="content-header">
            <h1>{{ trans('book.books_manager_deleted') }}</h1>
                            ...
                            <table class="table table-hover text-center">
                                <tbody>
                                    <tr>
                                        ...
                                        <th>{{ trans('book.day') }}</th>
                                        <th>{{ trans('book.hour') }}</th>
                                    ...
                                    @foreach ($books as $book)
                                        <tr>
                                            ...
                                            <td>
                                                {{ date('d-m-Y', strtotime($book->deleted_at)) }}
                                            </td>
                                            <td>
                                                {{ date('H:i', strtotime($book->deleted_at)) }}
                                            </td>
                                            <td class="td general">
                                                <a href="{{ route('admin.book-restore', $book->id) }}"
                                                    title="{{ trans('book.restore') }}">
                                                    <i class="fa fa-refresh"></i>
                                                </a>
                                                <form id="delete_{{ $book->id }}"
                                                    action="{{ route('admin.hard-delete', $book->id) }}" method="POST"
